...
PREFS = common.Preferences(
  ('kinopoiskshow_pref_max_posters', S.KINOPOISK_PREF_DEFAULT_MAX_POSTERS),
  ('kinopoiskshow_pref_max_art', S.KINOPOISK_PREF_DEFAULT_MAX_ART),
  ('kinopoiskshow_pref_get_all_actors', S.KINOPOISK_PREF_DEFAULT_GET_ALL_ACTORS),
  ('kinopoiskshow_pref_imdb_support', S.KINOPOISK_PREF_DEFAULT_IMDB_SUPPORT),
  (None, None),
  ('kinopoiskshow_pref_imdb_rating', S.KINOPOISK_PREF_DEFAULT_IMDB_RATING),
  ('kinopoiskshow_pref_kp_rating', S.KINOPOISK_PREF_DEFAULT_KP_RATING))
...
class KinoPoiskRuShowsAgent(Agent.TV_Shows):
  name = 'KinoPoiskRuShows'
...
#        metadata.id = imdbId
  ...
  def parseInfoTableTagAndUpdateMetadata(self, page, metadata):
    ...
    """
    data = KinoPoiskRuShowsAgent.parser.parseTitlePage(page)
    summaryPrefix = ''
    if 'title' in data:
...
#        metadata.writers.add(writer)
  ...
  def parseStudioPageData(self, metadata, kinoPoiskId):
    ...
    """
    data = KinoPoiskRuShowsAgent.parser.fetchAndParseStudioPage(kinoPoiskId)
  ...
  def parseCastPageData(self, titlePage, metadata, kinoPoiskId):
    """ Parses people - mostly actors. Here (on this page)
        ...
        creating this movie.
    """
    data = KinoPoiskRuShowsAgent.parser.fetchAndParseCastPage(kinoPoiskId, PREFS.getAllActors)
